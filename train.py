...
from consource.dataloading.datamodules.source_sep_datamodule import SourceSepDataModule
...
if __name__ == "__main__":
    ...
    cli = MyLightningCLI(model_class=ConSource, datamodule_class=SourceSepDataModule, seed_everything_default=123,
                         run=False, save_config_callback=LoggerSaveConfigCallback, save_config_kwargs={"overwrite": True},)
    
    cli.instantiate_classes()

    if cli.config.log:
        logger = WandbLogger(project="ConSource", id=cli.config.resume_id)
